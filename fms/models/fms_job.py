...
class FMSJob(models.Model):
    ...
    def _auto_create_so_po(self):
        for job in self:
            # Only original lines that are not pending/rejected
    ...
    def _check_and_trigger_scope_approval(self, additional_lines):
        """
        Triggered when additional scope lines are saved.
        - Finds active approval rule.
        - If total exceeds threshold → create/update approval request.
        - If below threshold → auto-approve the lines.
        - Rejected lines stay on job but are blocked from billing.
        """
        for job in self:
            if not additional_lines:
                continue

            # Find the active approval rule (most specific first)
            ...
            if not rule:
                # No rule configured — auto-approve all additional lines
            ...
            # Calculate total of these additional lines
            ...
            # Check if a pending approval already exists for this job
    ...
    def _create_so_po_for_approved_additional(self, lines):
        """Creates SO/PO for additional lines that were auto-approved (below threshold)."""
        for job in self:
    ...
    def action_schedule(self): pass
    def action_start(self): pass
    def action_complete(self): pass
    def action_create_invoice(self): pass
...
class FMSJobLine(models.Model):
    ...
    def create(self, vals_list):
        ...
        # Auto-detect additional scope:
        # If parent job already has an SO → new line = additional scope
        for line in records:
        ...
        # Original lines → auto SO/PO
        if original_jobs:
            original_jobs._auto_create_so_po()

        # Additional lines → scope approval workflow

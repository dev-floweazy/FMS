...
class FMSRateCard(models.Model):
    ...
    product_id = fields.Many2one(
        ...
        domain=[('is_fms_service', '=', True)], required=True
    )
    ...
    def _compute_display_name(self):
        for rec in self:
            ...
            if rec.valid_from or rec.valid_to:
                name.append("%s → %s" % (rec.valid_from or '', rec.valid_to or ''))
    ...
    def get_rate_for_customer(self, partner_id, product_id, rate_date=False):
        ...
        date_domain = expression.OR([[('valid_from', '=', False)], [('valid_from', '<=', rate_date)]])
        domain = expression.AND([domain, date_domain])
        date_to_domain = expression.OR([[('valid_to', '=', False)], [('valid_to', '>=', rate_date)]])
